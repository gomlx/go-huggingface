...
// InternalStates holds intermediate outputs and variables of the SAM2 model.
// It is primarily used for testing and debugging, but can also be used for partial transfer learning
// (e.g. extracting image embeddings, sparse/dense prompt embeddings, or upscaled features).
type InternalStates struct {
	// IoUScores predicted for each mask. Shape: [batch_size, point_batch_size, num_masks] (values usually in [0, 1]).
	IoUScores *Node
	// FpnHiddenStates are the multi-scale features from the vision neck.
	...
	// IouTokenOut is the IoU token output from the transformer. Shape: [batch_size, point_batch_size, 256].
	IouTokenOut *Node
...
//   - pixelValues: Input image tensor of shape [batch_size, 3, 1024, 1024].
//     It should be normalized with ImageNet mean ([0.485, 0.456, 0.406]) and std ([0.229, 0.224, 0.225]).
//   - inputPoints: Input point coordinates of shape [batch_size, point_batch_size, num_points, 2].
//     The coordinates should be absolute pixel coordinates (e.g. from 0 to 1023). Can be nil if inputBoxes is provided.
...
//
// Outputs:
//   - predictionMask: The predicted mask logit tensor of shape [batch_size, point_batch_size, num_masks, 256, 256],
//     where num_masks is 3 if multimaskOutput is true, or 1 if false. Mask values are unnormalized logits.
//   - internalStates: Struct containing intermediate states and predictions (e.g. IoUScores, FpnHiddenStates) that are
//     mainly used for testing and can be ignored for normal inference.
func Forward(scope *model.Scope, pixelValues *Node, inputPoints, inputLabels, inputBoxes, inputMasks *Node, config *Config, multimaskOutput bool) (*Node, InternalStates) {
	...
	internalStates := InternalStates{
		IoUScores:        iouScores,
		FpnHiddenStates:  fpnHiddenStates,
		SparseEmbeddings: sparseEmbeds,
		DenseEmbeddings:  denseEmbeds,
		IouTokenOut:      iouTokenOut,
		MaskTokensOut:    maskTokensOut,
		Upscaled:         upscaled,
		Upscaled2:        upscaled2,
		HyperIn:          hyperIn,
	}
