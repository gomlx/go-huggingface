...
import (
	...
	"github.com/gomlx/go-huggingface/examples/gemma4-e4bit"
...
func TestGemmaDebug(t *testing.T) {
...
func TestGemmaTokenizerNewlines(t *testing.T) {
	repo, err := gemma4e4bit.LoadRepo()
	if err != nil {
		t.Fatalf("Failed to load repo: %v", err)
	}
...
func TestGemmaIncrementalDebug(t *testing.T) {
	backend, err := compute.New()
	if err != nil {
		t.Fatalf("Failed to initialize backend: %v", err)
	}
